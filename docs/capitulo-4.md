...
### 4.2.4. Bounded Context: System Monitoring & Control
...
### 4.2.5. Bounded Context: Plant Management
...
| **Categoría** | Aggregate Root                                      |
| **Propósito** | Programacion de riego para una planta en especifico |
...
| Nombre       | Tipo de dato     | Visibilidad | Descripción                                    |
|--------------|------------------|-------------|------------------------------------------------|
| WateringTime | `DateTime`       | private     | La hora programada para el riego               |
| Frequency    | `String`         | private     | La frecuencia de riego (diario, semanal, etc.) |
| Status       | `Status` (enum)  | private     | Estado de la programación (activa/inactiva).   |
| potId        | `unsigned long`  | private     | Referencia a la maceta asociada al riego       |
| CreatedDate  | `DateTime`       | private     | Fecha de creación de la programación           |
...
| Nombre         | Tipo de retorno | Visibilidad | Descripción                                    |
|----------------|-----------------|-------------|------------------------------------------------|
| activate       | `void`          | public      | Activa la programación de riego                |
| deactivate     | `void`          | public      | Desactiva la programación de riego             |
| updateSchedule | `void`          | public      | Actualiza el horario y la frecuencia del riego |

## WateringEvent

| Propiedad     | Valor                                         |
|---------------|-----------------------------------------------|
| **Nombre**    | WateringEvent                                 |
| **Categoría** | Aggregate Root                                |
| **Propósito** | Representa un evento de riego que ha ocurrido |
...
| Nombre       | Tipo de dato     | Visibilidad | Descripción                                          |
|--------------|------------------|-------------|------------------------------------------------------|
| wateredAt    | `DateTime`       | private     | La hora en que se realizó el riego                   |
| Status       | `Status` (enum)  | private     | El estado del riego (completo, fallido, cancelado).  |
| potId        | `unsigned long`  | private     | Referencia a la maceta que fue regada.               |
...
| Nombre     | Tipo de retorno | Visibilidad | Descripción                            |
|------------|-----------------|-------------|----------------------------------------|
| logEvent   | `void`          | public      | Registra el evento de riego realizado. |

## WateringSystem

| Propiedad     | Valor                                         |
|---------------|-----------------------------------------------|
| **Nombre**    | WateringSystem                                |
| **Categoría** | Aggregate Root                                |
| **Propósito** | Representa un evento de riego que ha ocurrido |
...
| **Categoría** | Controller                  |
| **Propósito** | Gestionar las programaciones de riego |
...
| Nombre                | Ruta                      | Acción                                | Handle                                             |
|-----------------------|---------------------------|---------------------------------------|----------------------------------------------------|
| CreateSchedule        | `/`                       | Crea una nueva programación de riego  | `CreateWateringScheduleCommand`                    |
| GetScheduleByPotId    | `/{potId}`                | Obtiene la programación de riego para una maceta | `GetWateringScheduleByPotIdQuery`                  |
| UpdateSchedule        | `/{potId}`                | Actualiza la programación de riego    | `UpdateWateringScheduleCommand`                    |
| DeleteSchedule        | `/{potId}`                | Elimina la programación de riego     | `DeleteWateringScheduleCommand`                    |
...
| Propiedad     | Valor                        |
|---------------|------------------------------|
| **Nombre**    | WateringScheduleService      |
| **Categoría** | Servicio                     |
| **Propósito** | Gestionar la programación de riego de las macetas |

### Métodos

| Nombre                      | Tipo de Retorno | Visibilidad | Descripción                                                   |
|-----------------------------|-----------------|-------------|---------------------------------------------------------------|
| createSchedule              | `void`          | `public`    | Crea una nueva programación de riego para una maceta.         |
| updateSchedule              | `void`          | `public`    | Actualiza la programación de riego para una maceta existente. |
| getScheduleForPot           | `WateringSchedule` | `public` | Obtiene la programación de riego para una maceta específica.  |
| deleteSchedule              | `void`          | `public`    | Elimina la programación de riego de una maceta.               |
...
| **Categoría** | Repositorio                                           |
| **Propósito** | Persistir y consultar entidades de `WateringSchedule` |
| **Interfaz**  | `IWateringScheduleRepository`                         |

### Métodos

| Nombre        | Tipo de Retorno    | Visibilidad | Descripción                                                |
|---------------|--------------------|-------------|------------------------------------------------------------|
| save          | `void`             | `public`    | Guarda una nueva programación de riego.                    |
| findByPotId   | `WateringSchedule` | `public`    | Obtiene la programación de riego de una maceta específica. |
| deleteByPotId | `void`             | `public`    | Elimina la programación de riego de una maceta.            |

## WateringEventRepository

| Propiedad     | Valor                                                    |
|---------------|----------------------------------------------------------|
| **Nombre**    | WateringEventRepository                                  |
| **Categoría** | Repositorio                                              |
| **Propósito** | Persistir y consultar eventos de riego (`WateringEvent`) |
| **Interfaz**  | `IWateringEventRepository`                               |

### Métodos

| Nombre      | Tipo de Retorno       | Visibilidad | Descripción                                            |
|-------------|-----------------------|-------------|--------------------------------------------------------|
| save        | `void`                | `public`    | Guarda un evento de riego.                             |
| findByPotId | `List<WateringEvent>` | `public`    | Obtiene los eventos de riego de una maceta específica. |
...
| **Nombre**    | Subscription                                               |
| **Categoría** | Entidad                                                    |
| **Propósito** | Representa la suscripción de un usuario a un plan premium. |

##### **Atributos**:

| Nombre             | Tipo de Dato    | Visibilidad | Descripción                                             |
|--------------------|-----------------|-------------|---------------------------------------------------------|
| **subscriptionId** | `unsigned long` | `private`   | Identificador único de la suscripción.                  |
| **userId**         | `unsigned long` | `private`   | Referencia al usuario que tiene la suscripción.         |
| **planType**       | `String`        | `private`   | El tipo de plan (por ejemplo, básico, premium).         |
| **startDate**      | `DateTime`      | `private`   | Fecha de inicio de la suscripción.                      |
| **endDate**        | `DateTime`      | `private`   | Fecha de finalización de la suscripción.                |
| **status**         | `String`        | `private`   | Estado de la suscripción (activa, cancelada, expirada). |
...
| **deactivate** | `void`          | `public`    | Desactiva la suscripción. |
...
| **Nombre**    | Payment                                                          |
| **Categoría** | Entidad                                                          |
| **Propósito** | Representa un pago realizado por un usuario para su suscripción. |

##### **Atributos**:

| Nombre             | Tipo de Dato    | Visibilidad | Descripción                                        |
|--------------------|-----------------|-------------|----------------------------------------------------|
| **paymentId**      | `unsigned long` | `private`   | Identificador único del pago.                      |
| **userId**         | `unsigned long` | `private`   | Referencia al usuario que realizó el pago.         |
| **subscriptionId** | `unsigned long` | `private`   | Referencia a la suscripción asociada al pago.      |
| **amount**         | `Decimal`       | `private`   | Monto pagado.                                      |
| **paymentDate**    | `DateTime`      | `private`   | Fecha en que se realizó el pago.                   |
| **paymentMethod**  | `String`        | `private`   | Método de pago (tarjeta de crédito, PayPal, etc.). |
| **status**         | `String`        | `private`   | Estado del pago (exitoso, fallido, pendiente).     |

##### **Métodos**:

| Nombre             | Tipo de Retorno | Visibilidad | Descripción        |
|--------------------|-----------------|-------------|--------------------|
| **processPayment** | `void`          | `public`    | Procesa el pago.   |
| **refund**         | `void`          | `public`    | Reembolsa el pago. |
...
| **Categoría** | Controller                             |
| **Propósito** | Gestionar las suscripciones de usuario |
...
| Nombre                | Ruta        | Acción                                      | Handle                       |
|-----------------------|-------------|---------------------------------------------|------------------------------|
| CreateSubscription    | `/`         | Crea una nueva suscripción                  | `CreateSubscriptionCommand`  |
| GetSubscriptionByUser | `/{userId}` | Obtiene la suscripción activa de un usuario | `GetSubscriptionByUserQuery` |
| UpdateSubscription    | `/{userId}` | Actualiza la suscripción de un usuario      | `UpdateSubscriptionCommand`  |
| DeleteSubscription    | `/{userId}` | Elimina la suscripción de un usuario        | `DeleteSubscriptionCommand`  |
...
| **Categoría** | Controller                                        |
| **Propósito** | Gestionar los pagos asociados a las suscripciones |
| **Ruta**      | `/api/payments`                                   |
...
| Nombre           | Ruta                  | Acción                                      | Handle                  |
|------------------|-----------------------|---------------------------------------------|-------------------------|
| CreatePayment    | `/`                   | Crea un nuevo pago para la suscripción      | `CreatePaymentCommand`  |
| GetPaymentByUser | `/{userId}`           | Obtiene el historial de pagos de un usuario | `GetPaymentByUserQuery` |
| RefundPayment    | `/{paymentId}/refund` | Procesa un reembolso para un pago           | `RefundPaymentCommand`  |
...
| Propiedad     | Valor                                       |
|---------------|---------------------------------------------|
| **Nombre**    | SubscriptionService                         |
| **Categoría** | Servicio                                    |
| **Propósito** | Gestionar las suscripciones de los usuarios |

### Métodos

| Nombre                     | Tipo de Retorno | Visibilidad | Descripción                                 |
|----------------------------|-----------------|-------------|---------------------------------------------|
| **createSubscription**     | `void`          | `public`    | Crea una nueva suscripción para un usuario. |
| **activateSubscription**   | `void`          | `public`    | Activa una suscripción.                     |
| **deactivateSubscription** | `void`          | `public`    | Desactiva la suscripción.                   |
| **renewSubscription**      | `void`          | `public`    | Renueva la suscripción de un usuario.       |
...
| Nombre                | Tipo de Retorno | Visibilidad | Descripción                                  |
|-----------------------|-----------------|-------------|----------------------------------------------|
| **processPayment**    | `void`          | `public`    | Procesa un pago para la suscripción.         |
| **refundPayment**     | `void`          | `public`    | Procesa un reembolso para un pago.           |
| **getPaymentHistory** | `List<Payment>` | `public`    | Obtiene el historial de pagos de un usuario. |
...
| **Categoría** | Repositorio                                       |
| **Propósito** | Persistir y consultar entidades de `Subscription` |
...
| Nombre                     | Tipo de Retorno   | Visibilidad | Descripción                                                   |
|----------------------------|-------------------|-------------|---------------------------------------------------------------|
| save                       | `void`            | `public`    | Guarda una nueva suscripción.                                 |
| findByUserId               | `Subscription`    | `public`    | Obtiene la suscripción activa de un usuario.                  |
| update                     | `void`            | `public`    | Actualiza una suscripción existente.                          |
| deleteByUserId             | `void`            | `public`    | Elimina la suscripción de un usuario.                         |
...
| **Categoría** | Repositorio                            |
| **Propósito** | Persistir y consultar pagos realizados |
...
| Nombre                     | Tipo de Retorno   | Visibilidad | Descripción                                                   |
|----------------------------|-------------------|-------------|---------------------------------------------------------------|
| save                       | `void`            | `public`    | Guarda un nuevo pago.                                         |
| findByUserId               | `List<Payment>`   | `public`    | Obtiene los pagos realizados por un usuario.                  |
| findByPaymentId            | `Payment`         | `public`    | Obtiene un pago específico.                                   |

## PaymentGatewayService

| Propiedad     | Valor                                                 |
|---------------|-------------------------------------------------------|
| **Nombre**    | PaymentGatewayService                                 |
| **Categoría** | Servicio                                              |
| **Propósito** | Interactúa con servicios externos para procesar pagos |

### Métodos

| Nombre         | Tipo de Retorno     | Visibilidad | Descripción                                                      |
|----------------|---------------------|-------------|------------------------------------------------------------------|
| processPayment | `TransactionStatus` | `public`    | Llama al servicio de pago externo para procesar una transacción. |
| refundPayment  | `void`              | `public`    | Llama al servicio externo para procesar un reembolso.            |
...
| Propiedad     | Valor                                                    |
|---------------|----------------------------------------------------------|
| **Nombre**    | CareRecommendation                                       |
| **Categoría** | Entidad                                                  |
| **Propósito** | Representa una recomendación de cuidado para una planta. |

##### **Atributos**:

| Nombre                    | Tipo de Dato    | Visibilidad | Descripción                                                          |
|---------------------------|-----------------|-------------|----------------------------------------------------------------------|
| **recommendationId**      | `unsigned long` | `private`   | Identificador único de la recomendación.                             |
| **plantId**               | `unsigned long` | `private`   | Referencia a la planta a la que se aplica la recomendación.          |
| **recommendationType**    | `String`        | `private`   | Tipo de recomendación (riego, fertilización, etc.).                  |
| **recommendationDetails** | `String`        | `private`   | Detalles específicos de la recomendación (ej., "riego cada 3 días"). |
| **createdDate**           | `DateTime`      | `private`   | Fecha en que se creó la recomendación.                               |

##### **Métodos**:

| Nombre                     | Tipo de Retorno  | Visibilidad  | Descripción                                            |
|----------------------------|------------------|--------------|--------------------------------------------------------|
| **generateRecommendation** | `void`           | `public`     | Genera una recomendación de cuidado para la planta.    |
| **updateRecommendation**   | `void`           | `public`     | Actualiza una recomendación existente.                 |
...
| **Nombre**    | PlantProfile                        |
| **Categoría** | Entidad                             |
| **Propósito** | Representa el perfil de una planta. |

##### **Atributos**:

| Nombre                    | Tipo de Dato    | Visibilidad | Descripción                                    |
|---------------------------|-----------------|-------------|------------------------------------------------|
| **plantId**               | `unsigned long` | `private`   | Identificador único de la planta.              |
| **species**               | `String`        | `private`   | Especie de la planta.                          |
| **idealTemperatureRange** | `String`        | `private`   | Rango de temperatura ideal para la planta.     |
| **idealHumidityRange**    | `String`        | `private`   | Rango de humedad ideal para la planta.         |
| **lightRequirement**      | `String`        | `private`   | Requerimiento de luz (baja, media, alta).      |
| **lastWateredDate**       | `DateTime`      | `private`   | Fecha en que se regó la planta por última vez. |

##### **Métodos**:

| Nombre            | Tipo de Retorno | Visibilidad | Descripción                                                    |
|-------------------|-----------------|-------------|----------------------------------------------------------------|
| **updateProfile** | `void`          | `public`    | Actualiza el perfil de la planta.                              |
| **getPlantNeeds** | `String`        | `public`    | Obtiene las necesidades de la planta (agua, luz, temperatura). |

---

#### 4.2.8.2. Interface Layer

## CareRecommendationController

| Propiedad     | Valor                                                      |
|---------------|------------------------------------------------------------|
| **Nombre**    | CareRecommendationController                               |
| **Categoría** | Controller                                                 |
| **Propósito** | Gestionar las recomendaciones de cuidado para las plantas. |
| **Ruta**      | `/api/care-recommendations`                                |

### Métodos

| Nombre                     | Ruta                  | Acción                                  | Handle                                |
|----------------------------|-----------------------|-----------------------------------------|---------------------------------------|
| CreateRecommendation       | `/`                   | Crea una nueva recomendación de cuidado | `CreateCareRecommendationCommand`     |
| GetRecommendationByPlantId | `/{plantId}`          | Obtiene la recomendación de una planta  | `GetCareRecommendationByPlantIdQuery` |
| UpdateRecommendation       | `/{recommendationId}` | Actualiza una recomendación existente   | `UpdateCareRecommendationCommand`     |
| DeleteRecommendation       | `/{recommendationId}` | Elimina una recomendación               | `DeleteCareRecommendationCommand`     |
...
| **Categoría** | Controller                             |
| **Propósito** | Gestionar los perfiles de las plantas. |
| **Ruta**      | `/api/plant-profiles`                  |

### Métodos

| Nombre                | Ruta         | Acción                                   | Handle                              |
|-----------------------|--------------|------------------------------------------|-------------------------------------|
| CreateOrUpdateProfile | `/`          | Crea o actualiza el perfil de una planta | `CreateOrUpdatePlantProfileCommand` |
| GetProfileByPlantId   | `/{plantId}` | Obtiene el perfil de una planta          | `GetPlantProfileByPlantIdQuery`     |
...
| Propiedad     | Valor                                                      |
|---------------|------------------------------------------------------------|
| **Nombre**    | CareRecommendationService                                  |
| **Categoría** | Servicio                                                   |
| **Propósito** | Gestionar las recomendaciones de cuidado para las plantas. |
...
| Nombre                                      | Tipo de Retorno      | Visibilidad | Descripción                                                 |
|---------------------------------------------|----------------------|-------------|-------------------------------------------------------------|
| **createRecommendation**                    | `void`               | `public`    | Crea una nueva recomendación para el cuidado de una planta. |
| **getRecommendationForPlant**               | `CareRecommendation` | `public`    | Obtiene la recomendación más reciente para una planta.      |
| **evaluateCareRecommendationEffectiveness** | `void`               | `public`    | Evalúa la efectividad de una recomendación dada.            |
...
| Nombre                    | Tipo de Retorno | Visibilidad | Descripción                                 |
|---------------------------|-----------------|-------------|---------------------------------------------|
| **createOrUpdateProfile** | `void`          | `public`    | Crea o actualiza el perfil de una planta.   |
| **getPlantProfile**       | `PlantProfile`  | `public`    | Obtiene el perfil de una planta específica. |

---

#### 4.2.8.4. Infrastructure Layer

## CareRecommendationRepository

| Propiedad     | Valor                                             |
|---------------|---------------------------------------------------|
| **Nombre**    | CareRecommendationRepository                      |
| **Categoría** | Repositorio                                       |
| **Propósito** | Persistir y consultar recomendaciones de cuidado. |
| **Interfaz**  | `ICareRecommendationRepository`                   |

### Métodos

| Nombre               | Tipo de Retorno            | Visibilidad | Descripción                                                     |
|----------------------|----------------------------|-------------|-----------------------------------------------------------------|
| save                 | `void`                     | `public`    | Guarda una nueva recomendación de cuidado.                      |
| findByPlantId        | `CareRecommendation`       | `public`    | Obtiene la recomendación de cuidado para una planta específica. |
| findHistoryByPlantId | `List<CareRecommendation>` | `public`    | Obtiene el historial de recomendaciones de una planta.          |
...
| **Categoría** | Repositorio                                |
| **Propósito** | Persistir y consultar perfiles de plantas. |
...
| Nombre        | Tipo de Retorno | Visibilidad | Descripción                                 |
|---------------|-----------------|-------------|---------------------------------------------|
| save          | `void`          | `public`    | Guarda o actualiza el perfil de una planta. |
| findByPlantId | `PlantProfile`  | `public`    | Obtiene el perfil de una planta específica. |

## WeatherAPIService

| Propiedad     | Valor                                                                                  |
|---------------|----------------------------------------------------------------------------------------|
| **Nombre**    | WeatherAPIService                                                                      |
| **Categoría** | Servicio                                                                               |
| **Propósito** | Interactúa con servicios externos para obtener el clima y ajustar las recomendaciones. |

### Métodos

| Nombre            | Tipo de Retorno | Visibilidad | Descripción                                             |
|-------------------|-----------------|-------------|---------------------------------------------------------|
| getCurrentWeather | `WeatherData`   | `public`    | Obtiene el clima actual para la ubicación de la planta. |
...
- **SensorRecord**: Representa un registro de datos de los sensores de la maceta.

| Atributo        | Tipo de Dato      | Visibilidad | Descripción                                            |
|-----------------|-------------------|-------------|--------------------------------------------------------|
| **recordId**    | `unsigned long`   | `private`   | Identificador único del registro de sensor.            |
| **potId**       | `unsigned long`   | `private`   | Referencia a la maceta de la que se obtuvo el dato.    |
| **sensorType**  | `String`          | `private`   | Tipo de sensor (temperatura, humedad, etc.).           |
| **sensorValue** | `Decimal`         | `private`   | Valor obtenido del sensor.                             |
...
| Nombre            | Tipo de Retorno      | Visibilidad | Descripción                                                      |
|-------------------|----------------------|-------------|------------------------------------------------------------------|
| **saveRecord**    | `void`               | `public`    | Guarda un nuevo registro de sensor.                              |
| **getSensorData** | `List<SensorRecord>` | `public`    | Obtiene los datos de sensor de una maceta en un rango de tiempo. |

- **DataInsight**: Representa un informe generado a partir de los datos de los sensores.

| Atributo           | Tipo de Dato    | Visibilidad | Descripción                                          |
|--------------------|-----------------|-------------|------------------------------------------------------|
| **insightId**      | `unsigned long` | `private`   | Identificador único del informe.                     |
| **potId**          | `unsigned long` | `private`   | Referencia a la maceta asociada al informe.          |
| **insightType**    | `String`        | `private`   | Tipo de informe (por ejemplo, tendencia de humedad). |
| **insightDetails** | `String`        | `private`   | Detalles del informe generado.                       |
| **createdDate**    | `DateTime`      | `private`   | Fecha en que se creó el informe.                     |

##### **Métodos**:

| Nombre              | Tipo de Retorno | Visibilidad | Descripción                                                    |
|---------------------|-----------------|-------------|----------------------------------------------------------------|
| **generateInsight** | `void`          | `public`    | Genera un nuevo informe a partir de los datos de los sensores. |
| **updateInsight**   | `void`          | `public`    | Actualiza un informe existente con nuevos datos.               |
...
| Propiedad     | Valor                                                                   |
|---------------|-------------------------------------------------------------------------|
| **Nombre**    | DataInsightController                                                   |
| **Categoría** | Controller                                                              |
| **Propósito** | Gestionar los informes generados a partir de los datos de los sensores. |
| **Ruta**      | `/api/data-insights`                                                    |

### Métodos

| Nombre                | Ruta               | Acción                             | Handle                           |
|-----------------------|--------------------|------------------------------------|----------------------------------|
| CreateInsight         | `/`                | Crea un nuevo informe de datos     | `CreateDataInsightCommand`       |
| GetInsightByPotId     | `/{potId}`         | Obtiene los informes de una maceta | `GetDataInsightByPotIdQuery`     |
| GetHistoricalInsights | `/{potId}/history` | Obtiene el historial de informes   | `GetHistoricalDataInsightsQuery` |
...
| Propiedad     | Valor                                                                                         |
|---------------|-----------------------------------------------------------------------------------------------|
| **Nombre**    | DataInsightService                                                                            |
| **Categoría** | Servicio                                                                                      |
| **Propósito** | Gestionar la creación y consulta de informes generados a partir de los datos de los sensores. |

### Métodos

| Nombre                    | Tipo de Retorno     | Visibilidad | Descripción                                              |
|---------------------------|---------------------|-------------|----------------------------------------------------------|
| **createInsight**         | `void`              | `public`    | Crea un nuevo informe a partir de los datos de sensores. |
| **getInsightForPot**      | `DataInsight`       | `public`    | Obtiene un informe generado para una maceta específica.  |
| **getHistoricalInsights** | `List<DataInsight>` | `public`    | Obtiene los informes históricos de una maceta.           |

## DataProcessingService

| Propiedad     | Valor                                                             |
|---------------|-------------------------------------------------------------------|
| **Nombre**    | DataProcessingService                                             |
| **Categoría** | Servicio                                                          |
| **Propósito** | Gestionar el procesamiento de datos para la creación de informes. |

### Métodos

| Nombre                | Tipo de Retorno | Visibilidad | Descripción                                                |
|-----------------------|-----------------|-------------|------------------------------------------------------------|
| **processSensorData** | `void`          | `public`    | Procesa los datos de los sensores para generar un informe. |
| **generateReport**    | `DataInsight`   | `public`    | Genera un informe sobre los datos de sensores.             |

---

#### 4.2.9.4. Infrastructure Layer

## DataInsightRepository

| Propiedad     | Valor                                     |
|---------------|-------------------------------------------|
| **Nombre**    | DataInsightRepository                     |
| **Categoría** | Repositorio                               |
| **Propósito** | Persistir y consultar informes generados. |
| **Interfaz**  | `IDataInsightRepository`                  |

### Métodos

| Nombre             | Tipo de Retorno     | Visibilidad | Descripción                                                  |
|--------------------|---------------------|-------------|--------------------------------------------------------------|
| save               | `void`              | `public`    | Guarda un nuevo informe generado.                            |
| findByPotId        | `DataInsight`       | `public`    | Obtiene los informes generados para una maceta específica.   |
| findHistoryByPotId | `List<DataInsight>` | `public`    | Obtiene el historial de informes para una maceta específica. |

## SensorDataRepository

| Propiedad     | Valor                                        |
|---------------|----------------------------------------------|
| **Nombre**    | SensorDataRepository                         |
| **Categoría** | Repositorio                                  |
| **Propósito** | Persistir y consultar los datos de sensores. |
| **Interfaz**  | `ISensorDataRepository`                      |

### Métodos

| Nombre      | Tipo de Retorno    | Visibilidad | Descripción                                               |
|-------------|--------------------|-------------|-----------------------------------------------------------|
| save        | `void`             | `public`    | Guarda un nuevo registro de sensor.                       |
| findByPotId | `List<SensorData>` | `public`    | Obtiene los registros de sensor de una maceta específica. |
